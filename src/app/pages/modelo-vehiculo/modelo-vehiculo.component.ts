...
export class ModeloVehiculoComponent implements OnInit {
  ...
  titulopant : string = "Modelo Vehiculo";
  icono : string = "pe-7s-flag";
    ...
    language: {
      ...
      select: {
        rows: {
          ...
          1: "Modelo Vehiculo seleccionado"
        }
      },
      paginate: {
        first: "Primero",
  ...
  ngOnInit() {
    this.api.validateSession('modelo-vehiculo');
  ...
  loadDataProceso() {
    const data_post = {
      ...
      p_mod_activo: this.p_mod_activo,
    };
  ...
  anulaMantenimiento(){
      if (this.dataanteriorseleccionada == '') {
        ...
        const dataPost = {
          p_mod_id : parseInt(this.dataanteriorseleccionada[0]),
          p_mod_activo : 0
        };
        swal.fire({
          title: 'Mensaje',
          ...
          if (result.isConfirmed) {
            this.api.getmodelovehiculoact(dataPost).subscribe((data: any) => {
              if(data[0].error == 0){
                swal.fire({
                  title: 'Exito',
  ...
  procesaRegistro(){
    if(this.descrip == ''){
      ...
      const dataPost = {
        p_mod_descri : this.descrip,
        p_mod_abrevi : this.abrev,
      };
      ...
      swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          this.api.getmodelovehiculoins(dataPost).subscribe((data: any) => {
            if(data[0].error == 0){
              swal.fire({
                title: 'Exito',
                text: data[0].mensa.trim(),
