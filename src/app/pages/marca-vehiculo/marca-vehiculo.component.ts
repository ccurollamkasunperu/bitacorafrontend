...
export class MarcaVehiculoComponent implements OnInit {
  ...
  titulopant : string = "Marca Vehiculo";
  icono : string = "pe-7s-light";
  ...
  ngOnInit() {
    this.api.validateSession('marca-vehiculo');
  ...
  loadDataProceso() {
    const data_post = {
      p_mar_id: this.p_mar_id,
      p_mar_activo: this.p_mar_activo,
    };
  ...
  anulaMantenimiento(){
      if (this.dataanteriorseleccionada == '') {
        ...
        const dataPost = {
          p_mar_id : parseInt(this.dataanteriorseleccionada[0]),
          p_mar_activo : 0
        };
        swal.fire({
          title: 'Mensaje',
          ...
          if (result.isConfirmed) {
            this.api.getmarcavehiculoact(dataPost).subscribe((data: any) => {
              if(data[0].error == 0){
                swal.fire({
                  title: 'Exito',
  ...
  procesaRegistro(){
    if(this.descrip == ''){
      ...
      const dataPost = {
        p_mar_descri : this.descrip,
        p_mar_abrevi : this.abrev,
      };
      ...
      swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          this.api.getmarcavehiculoins(dataPost).subscribe((data: any) => {
            if(data[0].error == 0){
              swal.fire({
                title: 'Exito',
                text: data[0].mensa.trim(),
