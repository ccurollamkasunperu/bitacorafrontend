...
export class RolesComponent implements OnInit {
  ...
  titulopant: string = "Roles";
  icono: string = "pe-7s-users";
  ...
  //VARIABLES QUE NO SE TOCAN - ES PARA TODOS LOS TS
  constructor(
  ...
  //FIN DE VARIABLES QUE NO SE TOCAN

  loadData() {
    const data_post = {
      p_rol_id: this.rol_id,
      p_rol_activo: this.rol_activo,
    };

    this.api.usuarioRolSel(data_post).subscribe((data: any) => {
      console.log(data);
      if (data.length != 0) {
        this.datarolesUsuario = data;
  ...
  /* anulaMantenimiento(){
      if (this.dataanteriorseleccionada == '') {
        ...
        const dataPost = {
          p1 : parseInt(this.dataanteriorseleccionada[0]),
          p2 : 0
        };
        swal.fire({
          title: 'Mensaje',
          ...
          if (result.isConfirmed) {
            this.api.g(dataPost).subscribe((data: any) => {
              if(data[0].error == 0){
                swal.fire({
                  title: 'Exito',
  ...
  procesaRegistro(){
    if(this.descrip == ''){
      ...
      const dataPost = {
        p_rol_descri : this.descrip,
        p_rol_abrevi : this.abrev,
        p_rol_siglas : this.siglas
      };
      ...
      swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          this.api.usuarioRolIns(dataPost).subscribe((data: any) => {
            if(data[0].error == 0){
              swal.fire({
                title: 'Exito',
                text: data[0].mensa.trim(),
