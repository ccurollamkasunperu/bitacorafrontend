...
@Component({
  selector: 'app-grupo-vehiculo',
  templateUrl: './grupo-vehiculo.component.html'
})
export class GrupoVehiculoComponent implements OnInit {
  ...
  titulopant : string = "Grupo Vehiculo"
  icono : string = "pe-7s-box1"
  ...
  ngOnInit() {
    this.api.validateSession('grupo-vehiculo');
  ...
  loadDataProceso() {
    const data_post = {
      p_grv_id: this.p_grv_id,
      p_grv_activo: this.p_grv_activo,
    };

    this.api.getgrupovehiculosel(data_post).subscribe((data: any) => {
      console.log(data);
  ...
  anulaMantenimiento(){
      if (this.dataanteriorseleccionada == '') {
        ...
        const dataPost = {
          p_grv_id : parseInt(this.dataanteriorseleccionada[0]),
          p_grv_activo : 0
        };
        swal.fire({
          title: 'Mensaje',
          ...
          if (result.isConfirmed) {
            this.api.getgrupovehiculoact(dataPost).subscribe((data: any) => {
              if(data[0].error == 0){
                swal.fire({
                  title: 'Exito',
  ...
  procesaRegistro(){
    if(this.descrip == ''){
      ...
      const dataPost = {
        p_grv_descri : this.descrip,
        p_grv_abrevi : this.abrev
      };
      ...
      swal.fire({
        ...
        if (result.isConfirmed) {
          this.api.getgrupovehiculoins(dataPost).subscribe((data: any) => {
            if(data[0].error == 0){
              swal.fire({
                title: 'Exito',
                text: data[0].mensa.trim(),
