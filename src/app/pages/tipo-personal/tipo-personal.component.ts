...
export class TipoPersonalComponent implements OnInit {
  ...
  titulopant : string = "Tipo Personal"
  ...
  ngOnInit() {
    this.api.validateSession('tipo-personal');
  ...
  loadDataProceso() {
    const data_post = {
      ...
      p_tip_activo : this.p_tip_activo
    };
  ...
  anulaMantenimiento(){
      if (this.dataanteriorseleccionada == '') {
        ...
        const dataPost = {
          p_tip_id : parseInt(this.dataanteriorseleccionada[0]),
          p_tip_activo : 0
        };
        swal.fire({
          title: 'Mensaje',
          ...
          if (result.isConfirmed) {
            this.api.gettipopersonalact(dataPost).subscribe((data: any) => {
              if(data[0].error == 0){
                swal.fire({
                  title: 'Exito',
  ...
  procesaRegistro(){
    if(this.descrip == ''){
      ...
      const dataPost = {
        p_tip_descri : this.descrip,
        p_tip_abrevi : this.abrev,
      };
      ...
      swal.fire({
        ...
        if (result.isConfirmed) {
          this.api.gettipopersonalins(dataPost).subscribe((data: any) => {
            if(data[0].error == 0){
              swal.fire({
                title: 'Exito',
                text: data[0].mensa.trim(),
