...
@Component({
  selector: 'app-motivo-parte-cab',
  templateUrl: './motivo-parte-cab.component.html'
})
export class MotivoParteCabComponent implements OnInit {
  ...
  titulopant : string = "Motivo Parte Cab.";
    ...
    language: {
      ...
      select: {
        rows: {
          ...
          1: "Motivo parte Cab. seleccionado"
        }
      },
      paginate: {
        first: "Primero",
  ...
  ngOnInit() {
    this.api.validateSession('motivo-parte-cab');
  ...
  anulaMantenimiento(){
      if (this.dataanteriorseleccionada == '') {
        ...
        const dataPost = {
          p_mpc_id : parseInt(this.dataanteriorseleccionada[0]),
          p_mpc_activo : 0
        };
        swal.fire({
          title: 'Mensaje',
          ...
          if (result.isConfirmed) {
            this.api.getmotivopartecabact(dataPost).subscribe((data: any) => {
              if(data[0].error == 0){
                swal.fire({
                  title: 'Exito',
  ...
  procesaRegistro(){
    if(this.descrip == ''){
      ...
      const dataPost = {
        p_mpc_descri : this.descrip,
        p_mpc_abrevi : this.abrev,
      };
      ...
      swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          this.api.getmotivopartecabins(dataPost).subscribe((data: any) => {
            if(data[0].error == 0){
              swal.fire({
                title: 'Exito',
                text: data[0].mensa.trim(),
