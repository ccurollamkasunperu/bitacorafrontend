...
@Component({
  selector: 'app-motivo-parte-det',
  templateUrl: './motivo-parte-det.component.html'
})
export class MotivoParteDetComponent implements OnInit {
  ...
  titulopant : string = "Motivo Parte Det.";
    ...
    language: {
      ...
      select: {
        rows: {
          ...
          1: "Motivo Parte Det. seleccionado"
        }
      },
      paginate: {
        first: "Primero",
  ...
  ngOnInit() {
    this.api.validateSession('motivo-parte-det');
  ...
  loadDataProceso() {
    const data_post = {
      ...
      p_mpd_activo: this.p_mpd_activo,
    };
  ...
  anulaMantenimiento(){
      if (this.dataanteriorseleccionada == '') {
        ...
        const dataPost = {
          p_mpd_id : parseInt(this.dataanteriorseleccionada[0]),
          p_mpd_activo : 0
        };
        swal.fire({
          title: 'Mensaje',
          ...
          if (result.isConfirmed) {
            this.api.getmotivopartedetact(dataPost).subscribe((data: any) => {
              if(data[0].error == 0){
                swal.fire({
                  title: 'Exito',
  ...
  procesaRegistro(){
    if(this.descrip == ''){
      ...
      const dataPost = {
        p_mpd_descri : this.descrip,
        p_mpd_abrevi : this.abrev,
      };
      ...
      swal.fire({
        ...
        if (result.isConfirmed) {
          this.api.getmotivopartedetins(dataPost).subscribe((data: any) => {
            if(data[0].error == 0){
              swal.fire({
                title: 'Exito',
                text: data[0].mensa.trim(),
