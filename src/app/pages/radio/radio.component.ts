...
export class RadioComponent implements OnInit{
  ...
  ngOnInit() {
    this.api.validateSession('radio');
  ...
  loadDataProceso() {
    const data_post = {
      p_rad_id: this.sel_rad_id,
      p_rad_activo: this.sel_rad_activo,
    };

    this.api.getradiosel(data_post).subscribe((data: any) => {
      console.log(data);
  ...
  anulaMantenimiento(){
      if (this.dataanteriorseleccionada == '') {
        ...
        const dataPost = {
          p_rad_id : parseInt(this.dataanteriorseleccionada[0]),
          p_rad_activo : 0
        };
        swal.fire({
          title: 'Mensaje',
          ...
          if (result.isConfirmed) {
            this.api.getradioact(dataPost).subscribe((data: any) => {
              if(data[0].error == 0){
                swal.fire({
                  title: 'Exito',
  ...
  procesaRegistro(){
    if(this.numero == ''){
      swal.fire({
        title: 'Error',
        text: 'Debe ingresar un número de Radio',
        icon: 'error',
        confirmButtonColor: '#3085d6',
        confirmButtonText: 'Aceptar',
      });
      ...
      const dataPost = {
        p_rad_numero : parseInt(this.numero),
      };
      ...
      swal.fire({
        ...
        if (result.isConfirmed) {
          this.api.getradioins(dataPost).subscribe((data: any) => {
            if(data[0].error == 0){
              swal.fire({
                title: 'Exito',
                text: data[0].mensa.trim(),
