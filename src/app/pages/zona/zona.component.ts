...
export class ZonaComponent implements OnInit {
  ...
  loadDataProceso() {
    const data_post = {
      p_zon_id: this.sel_sco_id,
      p_zon_activo: this.sel_sco_activo,
    };
  ...
  anulaMantenimiento(){
      if (this.dataanteriorseleccionada == '') {
        ...
        const dataPost = {
          p_zon_id : parseInt(this.dataanteriorseleccionada[0]),
          p_zon_activo : 0
        };
        swal.fire({
          title: 'Mensaje',
          ...
          if (result.isConfirmed) {
            this.api.getzonaact(dataPost).subscribe((data: any) => {
              if(data[0].error == 0){
                swal.fire({
                  title: 'Exito',
  ...
  procesaRegistro(){
    if(this.descrip == ''){
      ...
      const dataPost = {
        p_zon_descri : this.descrip,
        p_zon_abrevi : this.abrev,
        p_zon_colorh : this.color
      };
      ...
      swal.fire({
        ...
        if (result.isConfirmed) {
          this.api.getzonains(dataPost).subscribe((data: any) => {
            if(data[0].error == 0){
              swal.fire({
                title: 'Exito',
                text: data[0].mensa.trim(),
