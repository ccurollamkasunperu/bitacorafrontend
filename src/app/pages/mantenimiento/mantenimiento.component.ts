...
export class MantenimientoComponent implements OnInit {
  ...
  titulopant : string = "Mantenimiento"
  icono : string = "pe-7s-tools"
  ...
  ngOnInit() {
    this.api.validateSession('mantenimiento');
  }
      ...
      if (this.dataanteriorseleccionada == '') {
        ...
        swal.fire({
          ...
          if (result.isConfirmed) {
            /* this.api.get(dataPost).subscribe((data: any) => {
              if(data[0].error == 0){
                swal.fire({
                  title: 'Exito',
