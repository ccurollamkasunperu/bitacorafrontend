...
export class ModalRedComponent implements OnInit {
  ...
  loadConector() {
    ...
    this.api.getconectorsel(data_post).subscribe((data: any) => {
      this.dataConector = data;
    });
  }
  
  loadTarjetaAdaptador() {
    const data_post = {
      p_tra_id: 0,
      p_mar_id: 0,
      p_tra_activo: 1,
    ...
    this.api.gettarjetaredadaptadorsel(data_post).subscribe((data: any) => {
      this.dataTarjetaAdaptador = data;
    });
  }
